  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center gap-2 mb-4">
        <span className="text-sm text-augustus-400">Filter by topic:</span>
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          <div className="card text-center py-12">
            ...
            <p className="text-augustus-400">Failed to load sites. Is the backend running?</p>
          </div>
        ) : data?.sites.length === 0 ? (
          <div className="card text-center py-12">
            <Globe className="w-12 h-12 text-augustus-600 mx-auto mb-4" />
            <p className="text-augustus-400">
              {filterTopicId 
          ...
          data?.sites.map((site) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <a
                    ...
                    className="text-sm text-augustus-400 hover:text-accent truncate flex items-center gap-1"
                  >
                    {site.url}
                    <ExternalLink className="w-3 h-3 flex-shrink-0" />
                  </a>
                  <div className="flex items-center gap-4 text-xs text-augustus-500 mt-1">
                    <span>Last fetched: {formatDate(site.last_fetched)}</span>
                    {site.last_error && (
              ...
              {testResult && testResult.siteId === site.id && (
                <div className={clsx(
                  'mt-4 pt-4 border-t border-augustus-800/50',
                    ...
                    <div className="space-y-1 pl-6">
                      {testResult.articles.slice(0, 3).map((article, i) => (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-sm text-augustus-400 hover:text-accent block truncate"
                        >
                          • {article.title}
                        </a>
                      ))}
                      {testResult.articles.length > 3 && (
                        <span className="text-sm text-augustus-500">
