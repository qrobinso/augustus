...
Choose **Breakout podcast** on the dashboard for the configurable workflow: a saved topic, typed subject, or source chapter; optional focus; a length (5, 10, or 20 minutes); and a cast. Dashboard and MCP/API requests use the usual progress, cancellation, transcript and audio controls without automatically joining the browser's playback queue. Source-based breakouts retain a link to the original episode.
